function [X_3D_Int,param] = getRawADCDataInt(param)
...
for l = 1:param.Nsweep
    for m = 1:param.Nr
        dechirpMixl = param.dechirpMix(:,m,l)-param.dechirpTar(:,m,l); % dechirp received target signal
...
X_3D_Int = complex(zeros(param.Nrfft,param.Nr,param.Nsweep));
for l = 1:param.Nsweep
    X_3D_fullsample_l = squeeze(X_3D_fullsample(:,:,l));
    X_3D_downsample_l = downsample(X_3D_fullsample_l,dsRate);
    X_3D_Int(:,:,l) = X_3D_downsample_l(1:param.Nrfft,:); % Keep first param.Nrfft data, and discard the remaining that is typically not target samples
end
end
