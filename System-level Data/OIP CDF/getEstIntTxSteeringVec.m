...
at_int_estClairvoyant = zeros(param.Nrfft,param.Nt,param.numInt); % esimation of at_int on different range-velocity bins 
for n = 1: param.Nrfft
    Y_rD_3D_n = Y_rD_3D(n,:,param.binVelococityTarget(param.targetIdx))';
    ...
    for q = 1:param.numInt
        ...
        at_int_est_q = reshape((param.Ar_int*b_q)'*y_rD_3D_n,[],1); % estimator of at_int_q = [(param.Ar_int*b_q)'*y_1; ...; (param.Ar_int*b_q)'*y_M]
        at_int_estClairvoyant(n,:,q) = at_int_est_q;
    end
    % Validation of estimator of at_int_q: Y_rD_3D_n approximately = sum_q kron(at_int_est_q,param.Ar_int_q)
    % 2 int case: Y_rD_3D_n_est = kron(at_int_est_record(n,:,1).',param.Ar_int(:,1)) + kron(at_int_est_record(n,:,2).',param.Ar_int(:,2));
end
end
